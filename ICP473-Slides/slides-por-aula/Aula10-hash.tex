...
\begin{frame}{Limitações do XOR em Funções de Hash}
    ...
    \begin{itemize}
        ...
        \item Isso permite que um atacante modifique a mensagem \textbf{sem que seja detectada mudanças na integridade} da informação.
        \item Conclusão: para garantir a integridade, precisamos de funções de hash criptográficas \textbf{resistentes a colisões}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Pré-imagem e Colisões em Funções de Hash}
    \begin{itemize}
        ...
        \item \textbf{Colisão}: ocorre se existirem $x \neq y$ tal que $H(x) = H(y)$.
...
\begin{frame}{Pré-imagens e Potenciais Colisões}
    \begin{itemize}
        \item Suponha uma função de hash $H$ com \textbf{saída} de $n$ bits e \textbf{entrada} de $b$ bits ($b > n$).
        \item Total de mensagens de entrada possíveis: $2^b$.
        ...
        \item Apesar disso, os riscos de segurança não são tão graves; é necessário \textbf{definir requisitos} precisos de segurança para funções de hash criptográficas.
    \end{itemize}
\end{frame}
...
\begin{frame}{Requisitos de Funções de Hash}
    ...
    \end{figure}
    As primeiras três
    propriedades são requisitos \textbf{básicos} de uma função de hash.
\end{frame}

\begin{frame}{Resistência à Pré-imagem (Propriedade de Mão Única)}
    \begin{itemize}
        \item A resistência à pré-imagem significa que:
              \begin{enumerate}
                  \item É fácil gerar o código de hash a partir da mensagem.
        ...
        \item Geralmente esse requisito é essencial quando a autenticação envolve um valor secreto que não é transmitido
    \end{itemize}
\end{frame}

\begin{frame}{Resistência à Segunda Pré-imagem}
    \begin{itemize}
        \item Garante que é \textbf{impossível} encontrar uma mensagem alternativa com o mesmo valor de hash de uma mensagem específica.
        ...
        \item Se a função de hash não possuir essa propriedade, um invasor poderia:
              ...
              \end{itemize}
        \item Portanto, esta propriedade protege contra \textbf{ataques de falsificação}.
        \item Ou seja, é importante considerar este aspecto para prevenção contra falsificação quando se usa um hash encriptado.
    \end{itemize}
\end{frame}

\begin{frame}{Funções Hash Fraca e Forte}
    \begin{itemize}
        \item Se uma função de hash \textbf{somente} satisfaz as primeiras cinco propriedades é chamada de \textbf{função hash fraca}.
        ...
        \item \textbf{Motivo}: Função hash forte protege contra ataques onde terceiros tentam gerar mensagens diferentes com o mesmo hash.
    \end{itemize}
\end{frame}

\begin{frame}{Pseudoaleatoriedade em Funções de Hash Criptográficas}
    \begin{itemize}
        \item A pseudoaleatoriedade \textbf{não é tradicionalmente citada como requisito formal}, mas é implicitamente importante.
        ...
        \item \textbf{Ou seja}: é apropriado considerar que uma função de hash produz uma saída pseudoaleatória.
    \end{itemize}
\end{frame}
...
\begin{frame}{Relação entre propriedades de Funções de Hash}
    ...
    \begin{itemize}
        ...
        \item  Uma função que é resistente à colisão \textbf{também} é resistente à segunda pré-imagem, \textbf{mas o inverso não é necessariamente verdadeiro}.
...
\begin{frame}{Ataques de Força Bruta em Funções de Hash}
    ...
    \begin{itemize}
        \item  \textbf{Não dependem} do algoritmo de hash: dependem \textbf{apenas do tamanho do valor de hash} em bits.
        \item Consiste em tentar todas as combinações possíveis até encontrar uma pré-imagem ou colisão.
        \item O esforço necessário cresce exponencialmente com o tamanho do hash:
              \begin{itemize}
                  \item Para um hash de \(n\) bits: há \(2^n\) possíveis valores.
    ...
    \textbf{Lembrando}:
    \begin{itemize}
        \item Força bruta é diferente da criptoanálise
        \item Criptoanálise explora vulnerabilidades específicas do algoritmo
    \end{itemize}
\end{frame}
\begin{frame}{Ataques de Pré-imagem e Segunda Pré-imagem}
    \textbf{Cenário}: O atacante que conhece uma saída da Hash deseja:
    \begin{itemize}
        \item O adversário busca um valor \(y\) tal que \(H(y) = h\), onde \(h\) é um hash conhecido.
        \item Esse ataque explora a \textbf{dificuldade de inverter} a função de hash (resistência à pré-imagem)
        \item Método de \textbf{força bruta}:
              \begin{itemize}
                  \item Testar valores aleatórios de \(y\) até encontrar uma correspondência.
...
\begin{frame}{Ataques Resistentes à Colisão}
    \textbf{Cenário}: O atacante busca desobrir \textbf{pares de saída} da Hash
    \begin{itemize}
    ...
    \end{itemize}
    Lembra do \textbf{paradoxo do aniversário}?
    \begin{itemize}
        \item Dado um grupo de 23 pessoas escolhidas aleatoriamente, a chance de que duas pessoas terão a mesma data de aniversário é de mais de 50%.
        \item Em hashes a probabilidade de colisão aumenta rapidamente com o número de tentativas.
    \end{itemize}
...
\begin{frame}{Ataque de Colisão Explorado via Paradoxo do Aniversário}
    ...
    \begin{itemize}
        \item Preparação na origem: mensagem legítima \(x\) é criada
    ...
    \begin{itemize}
        ...
        \item Hashes de 160 bits (como SHA-1) aumentam a resistência:
              \begin{itemize}
                  ...
                  \item Contudo, com a evolução tecnológica: 160 bits começa a não ser mais seguro.
              \end{itemize}
...
\begin{frame}{Estrutura Iterativa de Funções de Hash}
    ...
    \begin{itemize}
        \item Estrutura iterativa de hash foi proposta por Merkle [MERK79, MERK89]
        \item É usada na maioria das funções de hash atuais, incluindo SHA.
        \item Funcionamento geral:
              \begin{itemize}
                  \item A mensagem de entrada é dividida em $L$ blocos de tamanho fixo $b$ bits.
                  \item O bloco final é preenchido para completar $b$ bits (Se necessário)
                  \item Inclui também o tamanho total da mensagem.
                        \begin{itemize}
...
\begin{frame}{Estrutura geral de hash seguro}
    \begin{figure}
        \centering
        \includegraphics[width=0.85\linewidth]{Figuras/estrutura-gera-hash-seguro.png}
    \end{figure}
    O algoritmo usa repetidamente uma \textbf{função de compactação} $f$:
    \begin{itemize}
        \item Duas entradas: (i) a \textbf{variável de encadeamento} (n bits da etapa anterior) e  (ii) bloco atual (b bits).
        \item Produz uma saída de $n$ bits. Normalmente, b $>$ n;
        \item Variável de encadeamento inicial é \textbf{definida pelo algoritmo} e o valor final dela é o \textbf{hash resultante}.
    \end{itemize}
\end{frame}
...
\end{frame}
\begin{frame}{Criptoanálise em funções de Hash}
...
\begin{frame}{Colisões em Funções de Hash}
    \begin{itemize}
        ...
        \item \textbf{Importante}: A segurança é definida pelo \textbf{esforço necessário para descobrir uma colisão, não pela sua inexistência}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Lógica do SHA-512}
    \begin{itemize}
        \item \textbf{Entrada}: mensagem com um tamanho máximo de menos de $2^{128}$ bits
        \item \textbf{Processamento}: A entrada é processada em blocos de 1024 bits.
        \item \textbf{Saída}: Resumo da mensagem de 512 bits.
    \end{itemize}

    O processamento consiste nas seguintes etapas:
    \begin{enumerate}
        \item Anexar bits de preenchimento.
        \item Anexar tamanho.
        \item Inicializar buffer de hash.
        \item Processar mensagem em blocos de 1024 bits
    \end{enumerate}

\end{frame}

\begin{frame}{Processamento SHA-512}
    \begin{figure}
        \centering
        \includegraphics[width=0.9\linewidth]{Figuras/sha-512-geracao-resumo-msg.png}
...
\begin{frame}{SHA-512: Etapa 2 - Anexar Tamanho}
    \textbf{Etapa 2 - Anexar tamanho:}
...
\begin{frame}{SHA-512:  Etapa 3 - Inicialização do Buffer de Hash}
    \begin{itemize}
        \item Um buffer de 512 bits mantém resultados intermediários e finais da função de hash.
        \item O buffer pode ser representado por 8 registradores de 64 bits, inicializados com seguintes valores hexadecimais ($64x8=512$):
              \begin{figure}
                  \centering
                  \includegraphics[width=\linewidth]{Figuras/buffer-hash-sha256.png}

              \end{figure}
        \item Esses valores são armazenados no formato big-endian, que é o byte mais significativo de uma word na posição de byte de endereço baixo (mais à esquerda).
        \item Essas words foram obtidas apanhando-se os primeiros sessenta e quatro bits das partes fracionárias das raízes quadradas dos oito primeiros números primos.
    \end{itemize}
\end{frame}

\begin{frame}{SHA-512:  Etapa 4 - Processar mensagem em blocos de 1024 bits}
    O núcleo do algoritmo SHA-512 processa a mensagem em blocos de \textbf{1024 bits} (128 words).
    Esse processamento consiste em um módulo de \textbf{80 rodadas}
    \begin{itemize}
        \item     Esse módulo é o que está rotulado como $F$ na figura.

    \end{itemize}


    \vspace{0.3cm}
    Cada rodada recebe como entrada o \textbf{buffer de 512 bits} $(a, b, c, d, e, f, g, h)$ e o atualiza.
    \begin{itemize}
        \item Na primeira rodada, o buffer contém o hash intermediário $H_{i-1}$.
        \item Cada rodada $t$ utiliza:
              \begin{itemize}
                  \item Um valor de 64 bits $W_t$, derivado do bloco atual $M_i$ (schedule da mensagem).
                  \item Uma constante aditiva,  $K_t$, onde $0 \leq t \leq 79$ indica uma das 80 rodadas
              \end{itemize}
    \end{itemize}

    \vspace{0.3cm}
    As constantes $K_t$ são os primeiros 64 bits das partes fracionárias das raízes cúbicas
    dos 80 primeiros números primos, oferecendo padrões pseudoaleatórios que reduzem
    regularidades na entrada.
\end{frame}

\begin{frame}{Processamento do SHA-512 em um único bloco}
    \begin{figure}
        \centering
        \includegraphics[width=0.7\linewidth]{Figuras/processamento-sha-512-unico-bloco.png}

    \end{figure}

\end{frame}

\begin{frame}{Constantes do SHA-512}
    \begin{figure}
        \centering
        \includegraphics[width=0.7\linewidth]{Figuras/constantes-sha512.png}

    \end{figure}

\end{frame}

\begin{frame}{SHA-512: Etapa 5 - Saída}
    ...
    \begin{figure}
        \centering
        \includegraphics[width=0.7\linewidth]{Figuras/resumo-comportamento-sha512.png}

    \end{figure}

\end{frame}

\begin{frame}{Função rodada do SHA-512}
    A função rodada recebe os valores $a,b,c,d,e,f,g,h$ e executa uma série de operações:
    \begin{itemize}
        \item Seis das oito words da saída da função round envolvem simplesmente permutação ($b, c, d, f, g, h$)
        \item Somente duas das words de saída (a, e) são geradas por substituição.
    \end{itemize}
    \begin{figure}
        \centering
        \includegraphics[width=0.8\linewidth]{Figuras/funcao-round-sha-512.png}
