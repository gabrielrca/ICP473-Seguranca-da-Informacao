\title{Aula 11 - Criptografia Assimétrica}

\author{Prof. Gabriel Rodrigues Caldas de Aquino}

\institute
{
  Instituto de Computação \\
  Universidade Federal do Rio de Janeiro\\
  gabrielaquino@ic.ufrj.br% Your institution for the title page
}
\date{Compilado em: \\ \today} % Date, can be changed to a custom date

%----------------------------------------------------------------------------------------
%    PRESENTATION SLIDE
%----------------------------------------------------------------------------------------



\begin{frame}
  % Print the title page as the first slide
  \titlepage
\end{frame}

\begin{frame}{Criptografia de Chave Pública}

  A criptografia de chave pública introduz uma mudança fundamental:
  \begin{itemize}
    \item Baseada em \textbf{funções matemáticas}, e não apenas em substituição e permutação.
    \item É \textbf{assimétrica}: utiliza duas chaves separadas (pública e privada), ao contrário da simétrica que usa uma única chave.
    \item Essa assimetria impacta diretamente a \textbf{confidencialidade}, a \textbf{distribuição de chaves} e a \textbf{autenticação}.
    \item Grande parte da teoria dos criptossistemas de chave pública é baseada na teoria dos números.
  \end{itemize}
\end{frame}
\begin{frame}{Criptografia de Chave Pública}

  Concepções equivocadas comuns:
  \begin{itemize}
    \item Não é intrinsecamente mais segura que a criptografia simétrica — a segurança depende do \textbf{tamanho da chave} e do \textbf{custo computacional} para quebrar a cifra.
    \item Não substitui a criptografia simétrica — mas apresenta todo uma gama de aplicações:
          \begin{itemize}
            \item \textbf{Gerenciamento de chaves}
            \item \textbf{Assinaturas digitais}
          \end{itemize}
  \end{itemize}
\end{frame}


\begin{frame}{Criptografia de Chave Pública - Termos comuns}
  \textbf{Chaves Assimétricas}:   Duas chaves relacionadas, uma \textbf{pública} e uma \textbf{privada}, usadas para operações complementares:
  \begin{itemize}
    \item Encriptação e decriptação
    \item Geração e verificação de assinatura
  \end{itemize}


  \textbf{Certificado de Chave Pública}: Documento emitido e assinado digitalmente pela chave privada de uma \textbf{Autoridade de Certificação (CA)}:
  \begin{itemize}
    \item Vincula o \textbf{nome de um assinante} a uma \textbf{chave pública}.
    \item Garante que o assinante identificado tem \textbf{controle exclusivo} da chave privada correspondente.
  \end{itemize}
\end{frame}
\begin{frame}{Criptografia de Chave Pública - Termos comuns}
  \textbf{Algoritmo Criptográfico Assimétrico}: Algoritmo criptográfico que usa duas chaves relacionadas:
  \begin{itemize}
    \item Uma chave \textbf{pública} e uma chave \textbf{privada}.
    \item É computacionalmente inviável derivar a chave privada a partir da pública.
  \end{itemize}


  \textbf{Infraestrutura de Chave Pública (PKI)}: Conjunto de políticas, processos e plataformas para administração de certificados e pares de chaves:
  \begin{itemize}
    \item Emissão, manutenção e revogação de certificados.
    \item Envolve servidores, software e estações de trabalho.
    \item Suporte para autenticação, confidencialidade e integridade.
  \end{itemize}
\end{frame}

\begin{frame}{Como Surgiu a Criptografia Assimétrica}
  O conceito de criptografia de chave pública evoluiu de uma tentativa de atacar \textbf{dois dos problemas mais difíceis associados à encriptação simétrica}:

  \begin{itemize}
    \item  \textbf{A distribuição de chaves} sob a encriptação simétrica:
          \begin{itemize}
            \item  Requer que dois comunicantes já compartilhem uma chave, que de alguma forma foi distribuída a eles; \textbf{OU}
            \item Requer o uso de um centro de distribuição de chaves - Diffie [DIFF88]:
                  “afinal, qual seria a vantagem de desenvolver criptossistemas impenetráveis, se seus usuários fossem forçados a
                  compartilhar suas chaves com um CDC que poderia ser comprometido por roubo ou suborno?”
          \end{itemize}
    \item  \textbf{Assinaturas digitais} - Se o uso da criptografia tivesse que se tornar comum, não apenas nas
          situações militares, mas para fins comerciais e particulares, então as mensagens e documentos eletrônicos precisariam do equivalente das assinaturas usadas nos documentos em papel.
  \end{itemize}
\end{frame}

\begin{frame}{Criptossistemas de chave pública}
  Os algoritmos assimétricos contam com uma chave para encriptação e uma chave diferente, porém relacio  nada, para a decriptação. Eles têm a seguinte característica importante:
  \begin{itemize}
    \item É computacionalmente inviável determinar a chave de decriptação dado apenas o conhecimento do   algoritmo de criptografia e da chave de encriptação.
    \item Além disso, alguns algoritmos, como RSA, também exibem esta característica:  Qualquer uma das duas chaves relacionadas pode ser usada para encriptação, com a outra para a decriptação.
  \end{itemize}


\end{frame}

\begin{frame}{Encriptação com chave pública}

  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figuras/encriptacao-com-chave-publica.png}

  \end{figure}

\end{frame}

\begin{frame}{Encriptação com chave privada}

  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figuras/encriptacao-com-chave-privada.png}

  \end{figure}

\end{frame}

\begin{frame}{Etapas Essenciais da Criptografia de Chave Pública}
  \begin{enumerate}
    \item Cada usuário gera um \textbf{par de chaves} (pública e privada) para encriptação e decriptação.
    \item A \textbf{chave pública} é disponibilizada em um repositório ou arquivo acessível; a \textbf{chave privada} permanece secreta.
    \item Para enviar uma mensagem confidencial a Alice, Bob usa a \textbf{chave pública de Alice} para encriptar a mensagem.
    \item Ao receber, Alice utiliza sua \textbf{chave privada} para decriptar. Apenas ela consegue recuperar o texto original.
  \end{enumerate}
\end{frame}


\begin{frame}{Esquema de Encriptação de Chave Pública}
  Um esquema de encriptação de chave pública possui cinco elementos principais:
  \begin{enumerate}
    \item \textbf{Texto claro}: mensagem ou dados legíveis fornecidos como entrada.
    \item \textbf{Algoritmo de encriptação}: aplica transformações no texto claro.
    \item \textbf{Chaves pública e privada}: par de chaves; se uma encripta, a outra decripta.
    \item \textbf{Texto cifrado}: mensagem embaralhada gerada como saída, dependente da chave.
    \item \textbf{Algoritmo de decriptação}: reverte o processo, recuperando o texto claro original.
  \end{enumerate}
\end{frame}

\begin{frame}{Criptossitema de chave pública - Objetivo Sigilo}

  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figuras/criptossistema-chave-publica.png}

  \end{figure}

\end{frame}

\begin{frame}{Criptossitema de chave pública - Objetivo autenticação}

  \begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{Figuras/criptossistema-chave-publica-autenticacao.png}

  \end{figure}

\end{frame}

\begin{frame}{Autenticação e Confidencialidade com Chave Pública}
  \textbf{Importante}:
  \begin{itemize}
    \item Apenas a encriptação com a chave privada do emissor garante \textbf{autenticação}, mas não a confidencialidade.
    \item Para prover \textbf{ambos}, aplica-se uma dupla encriptação:
  \end{itemize}

  \[
    Z = E(PU_b, \; E(PR_a, \; X))
  \]
  \[
    X = D(PU_a, \; D(PR_b, \; Z))
  \]

  \begin{itemize}
    \item O emissor primeiro encripta com sua \textbf{chave privada} (assinatura digital).
    \item Em seguida, encripta novamente com a \textbf{chave pública do receptor}.
    \item O receptor, ao decriptar, recupera tanto a \textbf{confidencialidade} quanto a \textbf{autenticidade}.
    \item Desvantagem: exige 4 operações de criptografia assimétrica por mensagem.
  \end{itemize}
\end{frame}

\begin{frame}{Criptossitema de chave pública - Objetivo autenticação e sigilo}

  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figuras/criptossistema-de-chave-publica-autenticacao-e-sigilo.png}

  \end{figure}

\end{frame}

\begin{frame}{Aplicações de criptografia de chave pública}

  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figuras/aplicacoes-criptografia-chave-publica.png}

  \end{figure}
  \textbf{Obs}: DSS é a sigla para
  Digital Signature Standard, um padrão de assinatura digital que, em conjunto com o DSA (Digital Signature Algorithm) e o SHA (Secure Hash Algorithm), permite a criação de assinaturas digitais e verificação de integridade de dados, utilizando criptografia de chave pública de forma a garantir a autenticidade e a não-repudiação de uma transação
\end{frame}

\begin{frame}{Requisitos para Criptografia de Chave Pública}
  \begin{enumerate}
    \item Geração de par de chaves $(PU_b, PR_b)$ deve ser \textbf{computacionalmente fácil}.
    \item Encriptação: dado $PU_b$ e $M$, calcular $C = E(PU_b, M)$ deve ser \textbf{fácil}.
    \item Decriptação: dado $PR_b$ e $C$, calcular $M = D(PR_b, C)$ deve ser \textbf{fácil}.
    \item \textbf{Inviável} obter $PR_b$ a partir de $PU_b$.
    \item \textbf{Inviável} recuperar $M$ conhecendo apenas $PU_b$ e $C$.
    \item (Opcional) Ordem das chaves pode ser invertida:
          $M = D[PU_b, E(PR_b, M)] = D[PR_b, E(PU_b, M)]$.
  \end{enumerate}

  \vspace{0.3cm}
  \textbf{Observação:} Apenas poucos algoritmos atendem a todos os requisitos, como RSA, ECC, Diffie-Hellman e DSS.
\end{frame}
\begin{frame}{Função de Mão Única com Alçapão}
  \begin{itemize}
    \item Função de mão única: mapeia um domínio $X$ em um intervalo $Y$ de forma que:
          \[
            Y = f(X) \text{ é fácil de calcular, mas } X = f^{-1}(Y) \text{ é inviável de obter.}
          \]
    \item "Com alçapão": existe uma informação secreta que permite inverter a função de forma eficiente.
    \item Essencial para criptografia de chave pública: permite encriptação segura sem compartilhar a chave privada.
  \end{itemize}

\end{frame}

\begin{frame}{Complexidade de Funções Criptográficas}
  \begin{itemize}
    \item \textbf{Fácil}: problema resolvível em tempo polinomial $O(n^a)$ em função do tamanho da entrada $n$. Onde $a$ é uma constante fixa.
    \item Algoritmos polinomiais pertencem à classe P.
    \item \textbf{Inviável}: problema cujo esforço cresce mais rápido que o tempo polinomial, por exemplo, $O(2^n)$.
    \item Observação: medir complexidade pelo pior caso ou caso médio não é suficiente para criptografia.
    \item Criptografia exige que a função seja inviável de reverter para \textbf{praticamente todas as entradas}.
  \end{itemize}
\end{frame}
\begin{frame}{Função de Mão Única com Alçapão}
  \begin{itemize}
    \item Uma função que é fácil de calcular em uma direção e inviável na outra, a menos que informação adicional (alçapão) seja conhecida.
    \item Formalmente, uma família de funções reversíveis $f_k$:
          \begin{itemize}
            \item $Y = f_k(X)$ é fácil, se $k$ e $X$ forem conhecidos.
            \item $X = f_k^{-1}(Y)$ é fácil, se $k$ e $Y$ forem conhecidos.
            \item $X = f_k^{-1}(Y)$ é inviável, se apenas $Y$ for conhecido, sem $k$.
          \end{itemize}
    \item A construção de esquemas de \textbf{chave pública} depende de \textbf{encontrar funções de mão única com alçapão adequadas}.
  \end{itemize}
\end{frame}

\begin{frame}{Algoritmo RSA}
  \begin{itemize}
    \item O artigo de Diffie e Hellman (1976) desafiou a comunidade a criar algoritmos de chave pública. \href{https://dl.acm.org/doi/10.1145/1499799.1499815}{\textcolor{blue}{Referência}}
    \item Muitos algoritmos iniciais se mostraram falhos.
    \item Em 1977, Ron Rivest, Adi Shamir e Len Adleman (MIT) desenvolveram o esquema RSA. \href{https://dl.acm.org/doi/10.1145/359340.359342}{\textcolor{blue}{Referência}}
    \item Publicado em 1978, o RSA se tornou a técnica mais aceita e amplamente implementada para criptografia de chave pública.
  \end{itemize}
\end{frame}
\begin{frame}{Funcionamento do RSA}
  \begin{itemize}
    \item RSA é uma cifra de bloco: o texto claro e o texto cifrado são inteiros entre $0$ e $n-1$.
    \item Tamanho típico: $n \approx 1024$ bits (cerca de 309 dígitos decimais).
    \item Cada bloco de texto claro $M$ deve satisfazer $0 \le M < n$.
    \item Encriptação e decriptação:
          \[
            C = M^e \bmod n,
          \]
          \[
            M = C^d \bmod n = (M^e)^d \bmod n
          \]
    \item Chave pública: $PU = \{e, n\}$ \\
          Chave privada: $PR = \{d, n\}$
    \item Requisitos:
          \begin{enumerate}
            \item $M^{ed} \bmod n = M$ para todo $M < n$
            \item Calcular $M^e \bmod n$ e $C^d \bmod n$ é eficiente
            \item Conhecendo apenas $e$ e $n$, é inviável determinar $d$
          \end{enumerate}
  \end{itemize}
\end{frame}

\begin{frame}{Requisito de Inversão no RSA}
  \begin{itemize}
    \item Para garantir que $M^{ed} \bmod n = M$, é necessário que:
          \begin{itemize}
            \item os expoentes $e$ e $d$ sejam \textbf{inversos multiplicativos módulo} $\varphi(n)$,
            \item onde $\varphi(n)$ é a \textbf{função totiente de Euler}.
          \end{itemize}
    \item Para $n = pq$, com $p$ e $q$ primos:
          \[
            \varphi(n) = (p-1)(q-1)
          \]
    \item Relação fundamental:
          \[
            ed \equiv 1 \pmod{\varphi(n)} \quad \Rightarrow \quad d \equiv e^{-1} \pmod{\varphi(n)}
          \]
    \item O inverso multiplicativo \textbf{só existe} se $e$ for \textbf{coprimo com} $\varphi(n)$:
          \[
            mdc(e, \varphi(n)) = 1
          \]
    \item O valor de $d$ pode ser obtido com o \textbf{algoritmo de Euclides estendido},
          que encontra o inverso modular de $e$ em relação a $\varphi(n)$.
  \end{itemize}
\end{frame}

\begin{frame}{Inverso Multiplicativo no RSA}
  No RSA, a chave privada $d$ é escolhida como o inverso multiplicativo de $e$ módulo $\varphi(n)$:

  \[
    d \equiv e^{-1} \pmod{\varphi(n)}
  \]

  \begin{itemize}
    \item Isso significa que $d$ satisfaz:
          \[
            d \cdot e \equiv 1 \pmod{\varphi(n)}
          \]
    \item Exemplo: $e = 7$, $\varphi(n) = 160$
          \[
            23 \cdot 7 = 161 \equiv 1 \pmod{160}
          \]
    \item Portanto, $d = 23$ é o inverso multiplicativo de $e = 7$ módulo 160.

  \end{itemize}
\end{frame}

% PQ FUNCIONA 1
\begin{frame}{\textbf{Entendendo como funciona}: O que queremos garantir}
  No RSA:
  \begin{itemize}
    \item Mensagem \(M\)
    \item Chave pública \((n, e)\)
    \item Chave privada \((n, d)\)
  \end{itemize}

  Criptografia:
  \[
    C \equiv M^e \pmod{n}
  \]

  Descriptografia:
  \[
    M' \equiv C^d \pmod{n} = (M^e)^d \pmod{n}
  \]

  Objetivo:
  \[
    M' \equiv M \pmod{n}
  \]
\end{frame}

% PQ FUNCIONA 2
\begin{frame}{\textbf{Entendendo como funciona}: Por que $e$ e $d$ devem ser inversos módulo $\varphi(n)$}
  \begin{itemize}
    \item \(n = pq\), \(\varphi(n) = (p-1)(q-1)\)
    \item Teorema de Euler: \(M^{\varphi(n)} \equiv 1 \pmod{n}\) se \(mdc(M,n) = 1\)
    \item Escolhemos \(d\) tal que:
          \[
            e \cdot d \equiv 1 \pmod{\varphi(n)}
          \]
    \item Ou seja, existe \(k\) tal que \(ed = 1 + k\varphi(n)\)
  \end{itemize}
\end{frame}

% PQ FUNCIONA 3
\begin{frame}{\textbf{Entendendo como funciona}: Aplicando na descriptografia}
  \[
    M' = C^d = (M^e)^d = M^{ed} = M^{1 + k\varphi(n)} = M \cdot (M^{\varphi(n)})^k
  \]

  Pelo Teorema de Euler:
  \[
    (M^{\varphi(n)})^k \equiv 1^k \equiv 1 \pmod{n}
  \]

  Portanto:
  \[
    M' \equiv M \pmod{n}
  \]

  \textbf{Conclusão:} funciona porque \(e \cdot d \equiv 1 \mod \varphi(n)\)
\end{frame}

\begin{frame}{Segurança do RSA}
  \begin{itemize}
    \item \textbf{1. Estrutura das chaves:}
          \begin{itemize}
            \item Chave pública: $PU = \{e, n\}$
            \item Chave privada: $PR = \{d, n\}$
            \item Onde $n = p \cdot q$, e $p$ e $q$ são primos grandes e secretos
          \end{itemize}

          \vspace{0.3cm}

    \item \textbf{2. Se $p$ e $q$ forem descobertos:}
          \begin{itemize}
            \item É possível calcular o totiente de Euler:
                  \[
                    \varphi(n) = (p - 1)(q - 1)
                  \]
            \item E então obter o inverso modular de $e$:
                  \[
                    d \equiv e^{-1} \pmod{\varphi(n)}
                  \]
            \item Logo, o atacante consegue reconstruir a chave privada $d$
          \end{itemize}

          \vspace{0.3cm}

    \item \textbf{3. Conclusão:}
          \begin{itemize}
            \item A segurança do RSA depende da dificuldade de \textbf{fatorar $n$}.
            \item Se alguém conseguir obter $p$ e $q$, o sistema é completamente quebrado.
            \item Portanto, manter $p$ e $q$ secretos é \textbf{fundamental}.
          \end{itemize}
  \end{itemize}
\end{frame}

\begin{frame}{Escolha do valor $e$ no RSA}
  \begin{itemize}
    \item \textbf{Geralmente, escolhemos} $e = 65537 = 2^{16} + 1$
    \item É considerado o \textbf{“ponto ideal”} entre segurança e desempenho.
  \end{itemize}

  \vspace{0.5em}
  \textbf{Propriedades:}
  \begin{itemize}
    \item \textbf{Ímpar}: requisito para ser coprimo com $\varphi(n)$ na maioria dos casos.
    \item \textbf{Pequeno o bastante para ser rápido}:
          \begin{itemize}
            \item Tem apenas 17 bits.
            \item Exige pouquíssimas multiplicações na exponenciação modular.
          \end{itemize}
    \item \textbf{Grande o suficiente para ser seguro}:
          \begin{itemize}
            \item Evita ataques que exploram expoentes muito pequenos (como $e = 3$ ou $e = 17$).
          \end{itemize}
    \item \textbf{Forma binária simples}:
          \[
            e = 10000000000000001_2
          \]
          o que torna a operação de exponenciação modular bem eficiente.
  \end{itemize}
\end{frame}


\begin{frame}{Esquema RSA}
  \textbf{Ingredientes:}
  \begin{itemize}
    \item Dois números primos secretos: $p$ e $q$.
    \item Calcula-se $n = pq$ (público).
    \item Escolhe-se $e$ tal que $mdc(\varphi(n), e) = 1$ e $1 < e < \varphi(n)$ (público).
    \item Calcula-se $d \equiv e^{-1} \mod \varphi(n)$ (privado).
  \end{itemize}

  \textbf{Chaves:}
  \begin{itemize}
    \item Chave privada: $\{d, n\}$
    \item Chave pública: $\{e, n\}$
  \end{itemize}

  \textbf{Processo de encriptação e decriptação:}
  \begin{itemize}
    \item Usuário B quer enviar mensagem $M$ para A:
          \[
            C = M^e \mod n \quad \text{(encriptação)}
          \]
    \item Usuário A recebe $C$ e decripta:
          \[
            M = C^d \mod n
          \]
  \end{itemize}
\end{frame}
\begin{frame}{Tabela resumo dos parâmetros do RSA}

  \begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{Figuras/tabela-resumo-rsa.png}

  \end{figure}

\end{frame}
\begin{frame}{Exemplo Numérico do RSA}
  \textbf{Geração de chaves:}
  \begin{itemize}
    \item $p = 17$, $q = 11$
    \item $n = pq = 187$
    \item $\varphi(n) = (p-1)(q-1) = 160$
    \item Escolhe $e = 7$, $mdc(e, 160) = 1$
    \item Calcula $d \equiv e^{-1} \mod 160 \Rightarrow d = 23$
  \end{itemize}

  \begin{block}{Explicação}
    \begin{itemize}
      \item Exemplo: $e = 7$, $\varphi(n) = 160$
            \[
              23 \cdot 7 = 161 \equiv 1 \pmod{160}
            \]
      \item Portanto, $d = 23$ é o inverso multiplicativo de $e = 7$ módulo 160.

    \end{itemize}

  \end{block}

  \textbf{Chaves:}
  \begin{itemize}
    \item Pública: $PU = \{7, 187\}$
    \item Privada: $PR = \{23, 187\}$
  \end{itemize}
\end{frame}
\begin{frame}{Exemplo Numérico do RSA}
  \textbf{Encriptação de $M = 88$:}
  \[
    C = M^e \mod n = 88^7 \mod 187
  \]
  \[
    88^7 \mod 187 = (88 \times 88^2 \times 88^4) \mod 187 = (88 \times 77 \times 132) \mod 187 = 11
  \]

  \textbf{Decriptação:}
  \[
    M = C^d \mod n = 11^{23} \mod 187
  \]
  \[
    11^{23} \mod 187 = 88
  \]

  \textbf{Resultado:} A mensagem original $M = 88$ é recuperada com sucesso.
\end{frame}
\begin{frame}{Exemplo Simples numérico - RSA}

  \begin{figure}
    \centering
    \includegraphics[width=\linewidth]{Figuras/exemplo-rsa-numerico-simples.png}

  \end{figure}

\end{frame}

\begin{frame}{Processamento de múltiplos blocos usando RSA}

  \begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{Figuras/processamento-multiplos-blocos-rsa.png}

  \end{figure}

\end{frame}

\begin{frame}[fragile]{Usando e verificando os parâmetros RSA na prática}

  \begin{itemize}
    \small
    \item \textbf{Gerar a chave ssh}: \begin{verbatim}ssh-keygen -t rsa -f ./teste \end{verbatim}
    \item \textbf{Verificar fingerprint da chave}: \begin{verbatim}ssh-keygen -lf ./teste \end{verbatim}
    \item \textbf{Tentear abrir} (não vai funcionar): \begin{verbatim}openssl rsa -in ./teste -text -noout \end{verbatim}
    \item \textbf{Mudar o formato}: \begin{verbatim}ssh-keygen -p -m PEM -f ./teste \end{verbatim}
    \item \textbf{Verifiar fingerprint anterior}: \begin{verbatim}ssh-keygen -lf ./teste \end{verbatim}
    \item \textbf{Tentear abrir} (vai funcionar): \begin{verbatim}openssl rsa -in ./teste -text -noout \end{verbatim}
    \item \textbf{Mudar formato da chave pública para modo PEM}: \begin{verbatim}ssh-keygen -e -m PEM -f ./teste.pub > ./teste_pub.pem \end{verbatim}
    \item \textbf{Abrir a chave publica}: \begin{verbatim}openssl rsa -pubin -in teste_pub.pem -text -noout \end{verbatim}
  \end{itemize}

\end{frame}

\begin{frame}{O que é cada parâmetro de uma chave RSA}
  \footnotesize
  \begin{table}[h]
    \centering
    \begin{tabular}{lll}
      \toprule
      \textbf{Nome}         & \textbf{Símbolo}                  & \textbf{Função}                                       \\
      \midrule
      modulus               & $n = p \cdot q$                   & Base da aritmética modular — o número grande público. \\
      publicExponent        & $e$                               & Expoente público, normalmente 65537.                  \\
      privateExponent       & $d$                               & Expoente privado usado para assinar/decifrar.         \\
      prime1                & $p$                               & Primeiro primo secreto.                               \\
      prime2                & $q$                               & Segundo primo secreto.                                \\
      exponent1 / exponent2 & $d \bmod (p-1)$ e $d \bmod (q-1)$ & Para otimização - Teorema Chinês do Resto.            \\
      coefficient           & $q^{-1} \bmod p$                  & Outro parâmetro auxiliar do Teo. Chin. Resto.         \\
      \bottomrule
    \end{tabular}
  \end{table}
\end{frame}

\begin{frame}{O Problema da Fatoração no RSA}
  A segurança do RSA depende da dificuldade de certos problemas matemáticos.
  Existem três possíveis formas de ataque:

  \begin{enumerate}
    \item \textbf{Fatorar $n$:} encontrar os primos $p$ e $q$ tais que $n = p \times q$.
          Isso permite calcular $\varphi(n) = (p-1)(q-1)$ e, em seguida, $d \equiv e^{-1} \pmod{\varphi(n)}$.
    \item \textbf{Determinar $\varphi(n)$ diretamente:} sem fatorar $n$, mas que leva igualmente à descoberta de $d$.
    \item \textbf{Descobrir $d$ diretamente:} a partir de $e$ e $n$, sem precisar de $\varphi(n)$.
          Até hoje, parece tão difícil quanto fatorar $n$.
  \end{enumerate}

  \medskip
  \textbf{Conclusão:}
  \begin{itemize}
    \item A criptoanálise do RSA está fortemente ligada ao problema da \alert{fatoração de inteiros}.
    \item O desempenho dos melhores algoritmos de fatoração é usado como \textbf{benchmark de segurança}.
  \end{itemize}
\end{frame}

\begin{frame}{Dificuldade de Fatoração em Diferentes Tamanhos de Chave RSA}
  \begin{itemize}
    \item A dificuldade de fatorar cresce rapidamente com o tamanho do módulo:
          \begin{itemize}
            \item Fatorar um módulo RSA de \textbf{1024 bits} é cerca de \textbf{mil vezes mais difícil} do que um de 768 bits.
            \item Um módulo de \textbf{768 bits} é milhares de vezes mais difícil que um de 512 bits.
          \end{itemize}
    \item A primeira fatoração bem-sucedida de um módulo de \textbf{512 bits} ocorreu há cerca de uma década.
    \item É plausível que um módulo de \textbf{1024 bits} possa ser fatorado dentro da próxima década por grupos acadêmicos.
  \end{itemize}

  \medskip
  \textbf{Recomendação prática:}
  \begin{itemize}
    \item Evitar o uso de chaves RSA de 1024 bits nos próximos \textbf{3 a 4 anos}.
    \item Preferir módulos de pelo menos \alert{2048 bits} para segurança a médio prazo.
  \end{itemize}
\end{frame}


\begin{frame}{Troca de Chaves Diffie-Hellman}
  \begin{itemize}
    \item Primeiro algoritmo de chave pública, proposto por Diffie e Hellman em 1976.
    \item Objetivo: permitir que dois usuários troquem \textbf{variáveis} para criar uma chave secreta com segurança.
    \item Essa chave secreta pode ser usada posteriormente para criptografar mensagens.
    \item O algoritmo em si não criptografa mensagens, apenas realiza a \textbf{troca de valores secretos}.
    \item Baseado em propriedades da aritmética modular e do problema do logaritmo discreto.
  \end{itemize}
\end{frame}

\begin{frame}{Logaritmo Discreto e Diffie-Hellman}
  \begin{itemize}
    \item A segurança do algoritmo Diffie-Hellman depende da dificuldade de calcular \textbf{logaritmos discretos}.
    \item Definição: dado um número primo $p$ e uma raiz primitiva $a$, temos:
          \[
            a^1 \bmod p, \; a^2 \bmod p, \; \ldots, \; a^{p-1} \bmod p
          \]
          que geram todos os inteiros de $1$ até $p-1$.
    \item Para qualquer inteiro $b$, existe um único expoente $i$ tal que:
          \[
            b \equiv a^i \pmod{p}, \quad 0 \leq i \leq p-1
          \]
    \item Esse $i$ é o \textbf{logaritmo discreto} de $b$ na base $a$, módulo $p$:
          \[
            i = \log_a (b) \pmod{p}
          \]
    \item Problema: calcular $i$ é computacionalmente inviável para valores grandes de $p$.
  \end{itemize}
\end{frame}
\begin{frame}{Algoritmo de Troca de Chaves Diffie-Hellman}
  \begin{itemize}
    \item Dois parâmetros públicos:
          \[
            q \ (\text{número primo}), \quad \alpha \ (\text{raiz primitiva de } q)
          \]
    \item Cada usuário escolhe um valor secreto:
          \[
            A: X_A < q, \quad B: X_B < q
          \]
    \item Cada um calcula sua chave pública:
          \[
            Y_A = \alpha^{X_A} \bmod q, \quad Y_B = \alpha^{X_B} \bmod q
          \]
    \item As chaves públicas $Y_A$ e $Y_B$ são trocadas.
    \item Cálculo da chave secreta compartilhada:
          \[
            A: \ K = (Y_B)^{X_A} \bmod q
          \]
          \[
            B: \ K = (Y_A)^{X_B} \bmod q
          \]
    \item Como $(Y_B)^{X_A} \equiv (Y_A)^{X_B} \pmod{q}$, ambos obtêm a mesma chave secreta $K$.
  \end{itemize}
\end{frame}

\begin{frame}{Calculo da chave K - Diffie-Hellman}
  A chave K é calculada por ambas as partes da seguinte forma:
  \begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{Figuras/calculo-diffie-helman.png}

  \end{figure}
\end{frame}
\begin{frame}{Troca de Chaves Diffie-Hellman}
  O resultado é que os dois lados trocaram um valor secreto.
  Esse valor secreto é normalmente usado como chave secreta simétrica compartilhada.

  \medskip
  Agora, considere um intruso que pode observar a troca de chaves e queira determinar a chave secreta $K$.
  Como $X_A$ e $X_B$ são privados, o intruso só tem os seguintes elementos para trabalhar: $q$, $\alpha$, $Y_A$ e $Y_B$.

  \medskip
  Assim, o intruso é forçado a calcular um \textbf{logaritmo discreto} para estabelecer a chave.
  Por exemplo, para definir a chave privada do usuário B, o intruso precisaria calcular:
  \[
    X_B = \log_{\alpha}(Y_B) \pmod{q}
  \]
\end{frame}


\begin{frame}{Exemplo: Troca de Chaves Diffie-Hellman}
  A segurança da troca de chaves Diffie-Hellman está no fato de que, embora seja fácil calcular exponenciais
  módulo um primo, é muito difícil calcular logaritmos discretos. Para números grandes, esta tarefa é inviável.

  \medskip
  \textbf{Exemplo:}
  $q = 353$, $\alpha = 3$
  $X_A = 97$, $X_B = 233$

  \medskip
  \textbf{Cálculo das chaves públicas:}
  \[
    Y_A = 3^{97} \bmod 353 = 40 \quad\quad
    Y_B = 3^{233} \bmod 353 = 248
  \]

  \medskip
  \textbf{Cálculo da chave secreta comum:}
  \[
    K_A = (Y_B)^{X_A} \bmod 353 = 248^{97} \bmod 353 = 160
  \]
  \[
    K_B = (Y_A)^{X_B} \bmod 353 = 40^{233} \bmod 353 = 160
  \]

  \medskip
  \textbf{Para um intruso:}
  Disponível apenas $q=353$, $\alpha=3$, $Y_A=40$, $Y_B=248$.
  Para descobrir $K$, teria de resolver um \textit{logaritmo discreto}, o que é inviável para primos grandes.
\end{frame}

\begin{frame}{Diffie-Hellman na prática}
  \begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{Figuras/diffie-helman-alice-bob.png}

  \end{figure}
\end{frame}


\begin{frame}{Protocolos de Troca de Chave}
  \textbf{Troca simples (dois usuários):}
  \begin{itemize}
    \item $A$ escolhe $X_A$, calcula $Y_A = \alpha^{X_A} \bmod q$ e envia a $B$.
    \item $B$ escolhe $X_B$, calcula $Y_B = \alpha^{X_B} \bmod q$ e envia a $A$.
    \item Ambos calculam a chave secreta $K$:
          \[
            K = (Y_B)^{X_A} \bmod q = (Y_A)^{X_B} \bmod q
          \]
    \item Os valores públicos $q$ e $\alpha$ devem ser conhecidos previamente ou enviados na primeira mensagem.
  \end{itemize}

\end{frame}

\begin{frame}{Man-in-the-Middle - Diffie Hellman}
  \begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{Figuras/mitm-diffie-hellman.png}

  \end{figure}
\end{frame}

\begin{frame}{Ataque Man-in-the-Middle em Diffie–Hellman}


  \medskip
  \textbf{Consequência:}
  \begin{itemize}
    \item Alice e Bob acreditam ter uma chave secreta compartilhada.
    \item Na prática, Alice compartilha $K_2$ com Darth e Bob compartilha $K_1$ com Darth.
    \item Darth pode \textbf{espiar} ou \textbf{modificar} mensagens.
  \end{itemize}

  \medskip
  \textbf{Motivo da vulnerabilidade:}
  O protocolo não autentica os participantes.

  \textbf{Solução:} uso de \alert{assinaturas digitais} e \alert{certificados}.
\end{frame}
